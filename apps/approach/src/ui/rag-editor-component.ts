...
/**
 * Pure RAG Editor Business Component
 * 
 * Handles ONLY document editing functionality - NO authentication logic.
 * Authentication is handled by parent rag-bundle-app wrapper.
 */
@customElement('rag-editor-component')
export class RagEditorComponent extends SignalWatcher(LitElement) {
  @state() private statusMessage = '';
  @state() private statusType: 'info' | 'success' | 'error' = 'info';
  @state() private isLoading = false;
  @state() private editingDocument: any = null;
  @state() private documentContent = '';
  @state() private isTokenCalculationStale = false;
  @state() private originalDocumentContent = '';
  @state() private editingTags = '';
  @state() private editingCategory = '';
  @state() private editingIsA = '';
  @state() private editingChildOf = '';
  @state() private editingHasA = '';
  @state() private allDocuments: any[] = [];
  @state() private showDocumentList = true;
  @state() private isCreatingNew = false;
  @state() private newDocumentTitle = '';
  @state() private newDocumentFilename = '';
  ...
  // Public method that can be called by the parent bundle wrapper
  checkForDocumentData() {
  ...
  private getPostedDocumentData(): {id: string, document: string, metadata: any} | null {
    // Check if there's POST data in localStorage (set in query component)
  ...
  private editDocument(document: any) {
    ...
    this.editingIsA = metadata.is_a || '';
    this.editingChildOf = metadata.child_of || '';
    this.editingHasA = metadata.has_a || '';
  ...
  private async saveDocumentUpdate() {
    ...
    try {
      ...
      const updateOperation = updateDocumentAsync(requestData);
      ...
      if (updateOperation.value?.success) {
        ...
        this.originalDocumentContent = currentContent;
        this.documentContent = currentContent; // Sync component property
  ...
  private getCurrentContent(): string {
    // Get content from CodeMirror editor if available, fallback to component property
    const editorElement = this.shadowRoot?.querySelector('awr-markdown-codemirror') as any;
    const editorContent = editorElement?.editorView?.state.doc.toString();
    return editorContent || this.documentContent || '';
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'rag-editor-component': RagEditorComponent;
  }
}